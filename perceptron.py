...
class Perceptron():
	...
	def __init__(self):
		self.alpha = 0.3 #factor de aprendizaje. Cuanto mayor es antes aprende pero tiene menor precision
		self.perceptron = [] # array que nos define el perceptron y por cada capa contiene un subarray de la siguiente forma: [numero neuronas de la capa,[umbrales de actuacion(u)],[pesos(w)],[salidas de las neuronas(a)]]
		...
		while True: #utilizamos la red neuronal
			for i in range(self.perceptron[0][0]):
				self.perceptron[0][3][i] =float(input('entrada; has hecho un casting aqui a float ' + str(i) + ' : '))
			self.percep()
	...
	def train(self): #funcion que inicia el entrenamiento
		...
		for r in range(int(input('numero de veces que quiere repetir el entrenamiento: '))):
			self.readlines()
		self.save()
	...
	def backpropagation(self,se):
		#calculamos los errores y realizamos las correcciones oportunas
